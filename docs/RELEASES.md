...
**`app.json` version is the single source of truth.** Doc folders, PRRs, task lists, and git tags all use the same `MAJOR.MINOR.PATCH` number, prefixed with `alpha-v` for internal names.

| Thing | Example |
|---|---|
| `app.json` `version` | `1.1.0` |
| Doc folder / PRR / task list | `alpha-v1.1.0` |
| Git tag | `alpha-v1.1.0` |

**Before starting a new release:** bump `app.json` first, then create the docs folder to match.

Bump minor (`1.1.0 → 1.2.0`) for new features. Bump major (`1.x → 2.0.0`) for a new game or fundamental redesign. The `buildNumber` increments automatically per EAS build and is independent of the version.
