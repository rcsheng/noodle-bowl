...
**Content pattern:** `ContentProvider` → stale-while-revalidate (AsyncStorage cache + Firestore background refresh). Fallback to bundled `constants/data.ts` if both fail. If the cached version is from a previous day, `isLoading` stays `true` until Firestore delivers today's content — so game screens always see fresh questions on the first open of the day. `reload()` (called via `useFocusEffect` on the home tab) also sets `isLoading = true` before fetching, so if the user taps PLAY while a background refresh is in flight, the game screen waits for fresh content before picking its question.
...
**Deep link testing caveat:** Challenge and help URLs (`noodlebowl.app/c/TOKEN`, `/h/TOKEN`) do not embed the collection prefix. A link generated by `start:qa` writes to `qa_challenges` but any device without the prefix will look in `challenges` and fail. For cross-device deep link testing (Blocks 5, 6, 11), use `npm run start:prod_smoke` on both devices — this hits prod Firebase with no prefix so links round-trip correctly. ⚠️ This writes real data to prod `challenges` / `helpRequests` — use throwaway test accounts.
...
| Cross-device deep link testing (Blocks 5, 6, 11) | `npm run start:prod_smoke` | Production | `challenges`, `helpRequests` |
...
npm run start:qa           # smoke testing — prod Firebase, qa_ collections
npm run start:prod_smoke   # cross-device deep link testing — prod Firebase, no prefix (real collections)
